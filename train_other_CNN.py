...
"""
seq_ind = '0'
def write_to_file(strng,fname=""):
	fname="results_cnn_final_second"+str(seq_ind)+".txt"
	f = open(fname,'ab')
	f.write(strng+"\n")
	f.close()
...
def train_model_with_parallel_loading(model,q_train, X_test,Y_test, batch_size=8,load_at_once=1000, num_epoch=100, samples_train_count=13000,samples_test_count=7000, datagen = None,test_data_gen=None):
	...
	for i in range(num_epoch):
		...
		if((i+1) % 3 == 0):
			...
			model.save_model_params_dumb("/home/kundan/models/CNN_retrained_"+str(seq_ind)+"_no_shear_new_3drop_"+str(i+1)+"_"+str(avg_loss)+"_"+str(avg_acc)+".pkl")
	return model
...
if __name__ == '__main__':
	...
	seq_ind = train_samples_file.split('.')[0].split('second')[-1]
	print seq_ind
